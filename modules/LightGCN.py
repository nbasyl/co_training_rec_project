...
class GraphConv(nn.Module):
    ...
    def forward(self, user_embed, item_embed,
        ...
        # all_embed = torch.cat([user_embed.weight.data, item_embed.weight.data], dim=0)
...
class MFBPR(nn.Module):
  def __init__(self, data_config, args_config):
      ...
      self.user_embed = nn.Embedding(self.n_users, args_config.dim)
      self.item_embed = nn.Embedding(self.n_items, args_config.dim)
  ...
  def forward(self, batch=None):
      ...
      pos_i = self.item_embed(pos_item)
      neg_i = self.item_embed(neg_item[:, :self.K])


      return self.create_bpr_loss(user, pos_i, neg_i)


  def create_bpr_loss(self, user_emb, pos_embs, neg_embs):
    ...
    #   print(neg_e.shape)
      ...
      return mf_loss, mf_loss, mf_loss

  def get_user_item_embedding(self, batch=None, neg_k=8, supervised = True):
      user = batch['users']
      pos_item = batch['pos_items']
      neg_item = batch['neg_items']
      ...
      pos_i = self.item_embed(pos_item)
      if supervised == True:
        neg_i = self.item_embed(neg_item[:,:neg_k])
        return user, pos_i, neg_i
      else:
        neg_i = self.item_embed(neg_item[:,neg_k:])
        return user, neg_i
...
class LightGCN(nn.Module):
    ...
    def forward(self, net1=None, net2=None, batch=None):
        user = batch['users']
        pos_item = batch['pos_items']
        ...
        if self.ns == 'rns':  # n_negs = 1
            ...
            neg_s = self.pooling(neg_gcn_embs.view(-1, neg_gcn_embs.shape[2], neg_gcn_embs.shape[3]))
            user_s = self.pooling(user_gcn_emb[user])
            sample_ot1 = net1(torch.cat((user_s,neg_s),1))
            sample_ot2 = net2(torch.cat((user_s,neg_s),1))

            m = nn.LogSoftmax()
            res1 = torch.argmax(m(sample_ot1),dim=1)
            res2 = torch.argmax(m(sample_ot2),dim=1)

            result = (res1==1) == (res2==1)
            replace_idx = (result == False).nonzero(as_tuple=True)[0]
            neg_gcn_embs[replace_idx] = item_gcn_emb[neg_item[:, :self.K]][replace_idx]
    ...
    def get_user_item_embedding(self,user_gcn_emb, item_gcn_emb,batch=None, supervised = True):
      user = batch['users']
      pos_item = batch['pos_items']
      neg_item = batch['neg_items']

      def return_index_tensor(idicies, search_tensor):
        search_result = None
        for i, id in enumerate(idicies):
          if i == 0:
            search_result = search_tensor[id].view(1,search_tensor[id].shape[0])
          else:
            search_result = torch.vstack((search_result,search_tensor[id].view(1,search_tensor[id].shape[0])))

        return search_result

      # user_gcn_emb, item_gcn_emb = self.generate(split=True)
      user = return_index_tensor(user,user_gcn_emb)
      # pos_i = return_index_tensor(pos_item, item_gcn_emb)


      if supervised == True:
        pos_i = return_index_tensor(pos_item, item_gcn_emb)
        neg_i = return_index_tensor(neg_item[:,0], item_gcn_emb)
        # for i,j in enumerate(neg_item[:,:4]):
        #   if i == 0:
        #     # neg_i = torch.unsqueeze(torch.index_select(item_gcn_emb,0,j),0)
        #     print(j.shape)
        #     neg_res = return_index_tensor(j,item_gcn_emb)
        #     neg_i = neg_res.view(1,neg_res.shape[0],neg_res.shape[1])
        #   else:
        #     neg_res = return_index_tensor(j,item_gcn_emb)
        #     neg_res = neg_res.view(1,neg_res.shape[0],neg_res.shape[1])
        #     neg_i = torch.vstack((neg_i,neg_res))

        return user, pos_i, neg_i
      else:
        neg_i = None
        for i,j in enumerate(neg_item[:,-2:]):
          if i == 0:
            neg_res = return_index_tensor(j,item_gcn_emb)
            neg_i = neg_res.view(1,neg_res.shape[0],neg_res.shape[1])
          else:
